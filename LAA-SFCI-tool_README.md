...
2. Enter **LAA-VI** (required) and one or more LAA functional parameters
   (LAA-GLS and LAA-SLE are recommended as priority inputs).
...
| LAA-SLE | mm | LAA systolic longitudinal excursion |
| LAA-PEV / LAA-PFV / LAA-s′ | cm/s | Enter velocities in cm/s |
| LAA-FAC | % | Enter 28; a decimal such as 0.28 is also accepted |

> **Note on units:** the tool now uses the same unit conventions as the article — velocities in
> cm/s, LAA-SLE in mm, and LAA-FAC in %. Enter each value in the unit shown in the field hint.
> Note that only the dimensionless principal index (LAA-VI/LAA-GLS) is invariant to velocity units;
> the velocity- and displacement-based ratios change with the unit convention, so values must be
> entered in the units above for the embedded reference values and cutoffs to apply correctly.
